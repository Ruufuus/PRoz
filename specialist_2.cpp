    ...
    public:
                ...
                if(ack_count >= this->data.expert_count-1){
                    ...
                    for(int i = 0; i<this->process_count; i++){
                        if(this->process_list[i]>0){
                            ...
                            if(DEBUG)printf("[SPEC_2_WFS2REQ]\t%d\tWysyla MTAK2 zabiera proces %d!\n",this->process_id,i);
                            this->data.lamport_clock_value+=1;
        ...
        while(is_team_ready){
            ...
            if(status.MPI_TAG == S3IFREQ){
                ...
                if(DEBUG)printf("[SPEC_2_WFT]\t%d\tOtrzymuje TREADY od %d (%d %d %d)!\n",this->process_id, status.MPI_SOURCE, this->data.team_ids[0], this->process_id, status.MPI_SOURCE);
                this->data.lamport_clock_value = std::max(this->data.lamport_clock_value,message_buffor[0])+1;
                ...
                if(DEBUG)printf("[SPEC_2_WFT]\t%d\tOtrzymuje FTREADY od %d!\n",this->process_id, data.team_ids[2]);
        ...
        void ressurection(int rready_count){
            ...
            for(int i = 0; i < 3; i++){
                ...
                this->data.lamport_clock_value +=1;
                message = this->data.lamport_clock_value;
                if(DEBUG)printf("[SPEC_2_RESSURECT]\t%d\tWysyla RREADY do %d!\n",this->process_id, id);
